...
model.poly <- model_impute(
  aggr.better, 
  model.fun = glm, 
  rhs = "poly(Year, 4) + fPeriod", 
  model.args = list(family = poisson),
  extractor.fun = extractor.lm3,
  extractor.args = list(newdata = new.set)
)
model.poly <- cbind(new.set, model.poly)
model.poly$LCL <- qnorm(0.025, mean = model.poly$Estimate, sd = model.poly$SE)
model.poly$UCL <- qnorm(0.975, mean = model.poly$Estimate, sd = model.poly$SE)
ggplot(model.poly, aes(x = Year, y = Estimate, ymin = LCL, ymax = UCL)) + 
