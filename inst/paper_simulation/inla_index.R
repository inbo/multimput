singleRun <- function(run, path, seeds, n.imputation){
  ...
  output$"2.5 %" <- qnorm(0.025, mean = output$Estimate, sd = output$"Std. Error")
  output$"97.5 %" <- qnorm(0.975, mean = output$Estimate, sd = output$"Std. Error")
  output$Run <- this.run
...
to.do <- list.files(path, pattern = "^imp_[0123456789]{4}_0_0_0\\.rda$")
...
extra <- list.files(path, pattern = "^imp_[0123456789]{4}_[12]_[0123456789]_[0123456789]\\.rda$")
...
extra <- list.files(path, pattern = "^imp_[0123456789]{4}_(0_0|1_4)_0_(tr|rw|sl)\\.rda$")
extra <- gsub("^imp_", "", extra)
extra <- gsub("\\.rda$", "", extra)
extra <- sprintf(
  "%s_%01i",
...
done <- list.files(path, pattern = "^run_[0123456789]{4}_.*\\.rda$", full.names = TRUE)
...
save(results.inla, file = paste0(datadir, "/paper_inla.rda"))
