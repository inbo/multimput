...
#' @export
missingObserved <- function(dataset, count.variable = "Count", observed.variable = "Observed", site.variable = "Site", year.variable = "Year", period.variable = "Period"){
  ...
  pattern <- apply(combination, 1, function(x){
    selection <- waterfowl$Site == selected.sites[x["sites"]] & waterfowl$Period == selected.periods[x["periods"]]
    if(sum(selection) == 0){
      return(NULL)
    } else {
      data.frame(
  ...
  pattern[, site.variable] <- factor(pattern[, site.variable], levels = sort(unique(sites)), labels = levels(sites))
  pattern[, period.variable] <- factor(pattern[, period.variable], levels = sort(unique(periods)), labels = levels(periods))
  if(class(dataset[, year.variable]) == "factor"){
    pattern[, year.variable] <- factor(pattern[, year.variable], levels = sort(unique(dataset[, year.variable])), levels(dataset[, year.variable]))
  }
