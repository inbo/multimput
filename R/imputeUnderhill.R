...
imputeUnderhill <- function(data, formula = Observed ~ Year + Month + Site, initial = 0, family = c("nbinomial", "poisson")){
  ...
  while(do.loop){
    ...
    new.values <- round(predict(model, newdata = data[missing.data, ], type = "response"))
    if(any(data[missing.data, as.character(formula[2])] < new.values)){
      data[missing.data, as.character(formula[2])] <- pmax(data[missing.data, as.character(formula[2])], new.values)
