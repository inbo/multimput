...
#' @export
missingVolunteer <- function(dataset, proportion = 0.25, count.variable = "Count", observed.variable = "Observed", year.variable = "Year", site.variable = "Site", max.count = 100){
  ...
  probability.continu <- sapply(seq(2, length(years), by = 1), function(i){
    ...
    sqrt(pmin(observed.max[, count.variable], max.count) + 1) / sqrt(max.count + 1)
  })
  selected <- matrix(
    runif(
  ...
  for(i in seq(2, length(years), by = 1)){
    selected[, i] <- selected[, i] + selected[, i - 1] * probability.continu[, i - 1]
  }
  n <- sample(
    nrow(dataset),
